...
import OverflowY = Property.OverflowY
...
export default function calculateAutoSizeStyle(
  ...
  // the textbox
  const nodeType = calculateNodeStyling(uiTextNode, useCache)

  if (!nodeType) return {}

  const { paddingSize, borderSize, boxSizing, sizingStyle } = nodeType
  ...
  let overflowY: OverflowY | undefined
