...
function getPageXY(
  ...
  return obj ? obj[horizontal ? "pageX" : "pageY"] : 0
}
...
const ScrollBar = React.forwardRef<ScrollBarRef, ScrollBarProps>(
  (props, ref) => {
    ...
    React.useEffect(() => {
      ...
      thumbEle && thumbEle.addEventListener("touchstart", onThumbMouseDown)
      scrollbarEle &&
      ...
      return () => {
        ...
        thumbEle && thumbEle.removeEventListener("touchstart", onThumbMouseDown)
      }
    }, [])

    // Pass to effect
    const enableScrollRangeRef = React.useRef<number| null >(null)
    enableScrollRangeRef.current = enableScrollRange
    const enableOffsetRangeRef = React.useRef<number | null>(null)
    ...
    React.useEffect(() => {
      if (dragging) {
        ...
        const onMouseMove = (e: MouseEvent | TouchEvent) => {
          ...
          if (stateDragging) {
            const offset = getPageXY(e, horizontal) - (statePageY ?? 0)
            let newTop = stateStartTop ?? top
            ...
            const tmpEnableScrollRange = enableScrollRangeRef.current ?? 1
    ...
    const containerStyle: React.CSSProperties = {
      position: "absolute",
      visibility: visible && canScroll ? "visible" : "hidden",
    }
